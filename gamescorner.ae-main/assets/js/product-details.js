...
async function handleAddToCart(event) {
  ...
  if (!button) {
    showAlert('error', 'Could not find product information');
    return;
  }

  const product = button.dataset.product;
  const productPrice = parseFloat(button.dataset.productPrice);
  const productDiscount = parseFloat(button.dataset.productDiscount) || 0;
  const shippingPrice = parseFloat(button.dataset.shippingPrice) || 0;
  const taxAmount = parseFloat(button.dataset.taxAmount) || 0;
  const shippingTime = button.dataset.shippingTime || '';
