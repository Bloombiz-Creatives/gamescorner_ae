class OrderDetailsManager {
    ...
    initializeModal() {
        ...
        document.querySelector('.modal-close').addEventListener('click', this.modal.close);
    ...
    renderOrders() {
        ...
        if (!this.orderData || this.orderData.length === 0) {
            orderContainer.innerHTML = '<p class="text-center py-4">No orders found</p>';
            return;
        }

        const ordersHTML = this.orderData.map(order => this.createOrderCard(order)).join('');
                ...
                <div class="order-items-preview"  >
                    ${order.orderItems.map(item => `
                        <div class="order-item mb-3">
                            <a href="product-details.html?id=${item.product._id}">
                                <img src="${item.product.image || 'assets/images/thumbs/product-details-two-thumb2.png'}" 
                                     alt="${item.product.name}" 
                                     class="mb-2">
                            </a>
                            <div class="order-item-content">
                                <a href="product-details.html?id=${item.product._id}" class="font-medium">${item.product.name}</a>
                                <p>${this.formatCurrency(item.price, item.currency_code)}</p>
                                <p>Quantity: ${item.quantity}</p>
                            </div>
                            <div class="order-details-btn">
                            <button class="order-card order-btn-btn"  data-order-id="${order._id}">
    ...
    async handleCancelOrder(orderId) {
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/order/${orderId}/cancel`, {
                method: 'PUT',
                headers: {
                    "Authorization": `Bearer ${this.webtoken}`,
                    'Content-Type': 'application/json'
    ...
    showOrderDetails(order) {
        const showCancelButton = order.paymentInfo.status === 'Pending' && order.orderStatus === 'Processing';
        ...
        const detailsHTML = `
            <div class="order-details">
                ...
                <div class="shipping-address mb-4">
                    ...
                    <p>${order.shippingAddress.name}, Phone: ${order.shippingAddress.phoneNo}</p>
                ...
                <div class="order-summary">
                    ...
                    <div class="flex justify-between mb-1">
                        <span>Shipping:</span>
                        <span>${order.shippingPrice === 0 ? 'Free' : this.formatCurrency(order.shippingPrice)}</span>
                    </div>
                    <div class="flex justify-between mb-1">
                        <span>Tax:</span>
                ...
                <div class="payment-info mt-4 pt-4 border-t">
                    <p><strong>Payment Method:</strong> ${order.paymentInfo.type}</p>
        ...
        if (showCancelButton) {
            ...
            cancelBtn.addEventListener('click', () => this.handleCancelOrder(order._id));
        }
    }
