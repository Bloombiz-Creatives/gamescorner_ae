document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchWishlist() {
    try {
      const response = await fetch("https://api.gamescorner.ae/api/wish", {
        method: "GET",
        headers: {
                    ...
                    <div class="table-product d-flex align-items-center gap-24">
                        ...
                        <div class="table-product__content text-start">
                            <h6 class="title text-lg fw-semibold mb-8">
                                <a href="product-details.html?id=${
                                  product._id
                                }" class="link text-line-2" style="display: block; width:600px;">
      ...
      button.addEventListener("click", async (e) => {
        ...
        try {
          const response = await fetch(
            "https://api.gamescorner.ae/api/wish/remove",
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${localStorage.getItem("webtoken")}`,
  ...
  function attachClearWishlistListener() {
    if (clearCartButton) {
      // Ensure the clear cart button exists
      clearCartButton.addEventListener("click", async () => {
        try {
          const response = await fetch(
            "https://api.gamescorner.ae/api/wish/clear",
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${localStorage.getItem("webtoken")}`,
